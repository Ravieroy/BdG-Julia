module BdgUtilities
    ...
    """
    function create_t_matrix(nnMap; t=1, saveAsText=tMatSave, fileName=tMatFileName)
        nSites = length(nnMap)
    ...
    """
    function create_bdg_ham(deltaList, nSites, H, mu, nAvgOld, U, vList;)
        ...
        for i in nSites+1: 2 * nSites
            HBdG[i, i - nSites] = conj(deltaList[i - nSites])
        end
        return HBdG
    end
...
"""
    ...
    """
        run_self_consistency(deltaOld, mu, N, nAvgOld, nExp, lattice, seed, U, V; t = 1, tol=0.001, nLayer=1)
